            ...
            <div class="page-content" style="background-color: #1f2122;">
                <div class="cards-container col-6" style="background-color: #27292a;">
                    <el-breadcrumb separator="/">
                        ...
                        <el-breadcrumb-item>บันทึก รูปแบบ Simulator</el-breadcrumb-item>
                    </el-breadcrumb>
                </div>
                <div class="cards-container col-6" style="background-color: #27292a; width: auto; height: auto;">
                <form @submit.prevent="CreateSave" style="font-size: 19px; font-weight: bold; display: flex; flex-direction: column; justify-content: center; align-items: center;">
...
export default{
    ...
    methods: {
        async CreateSave() {
try {   
    // this.SaveSlotData.user_save_id = this.userid
    const apiURL = '/api_saveSlot/create-saveSlot';
    const response = await axios.post(apiURL, this.SaveSlotData);

    const SaveSlotData = response.data;
    console.log(SaveSlotData);
        ...
        axios.get(`http://localhost:4000/api_member/${this.userid}`)
            .then(response => {
                if (response.data && typeof response.data === 'object') {
                    this.SaveSlotData.user_save_id= response.data._id;
                }
            })
            .catch(error => {
