        ...
        <div class="col-lg-12">
            <div class="page-content" style="background-color: #1f2122;">
                ...
                <form @submit.prevent="uploadCreator_img" style="font-size: 19px; font-weight: bold; display: flex; flex-direction: column; justify-content: center; align-items: center;">
                        <div class=""> <h1 style="font-size: 60px; color: white; margin-left: 20px; margin-top: 20px;"><span class="color-yt">U</span>pload <span class="color-yt">I</span>mage <span class="color-yt">C</span>reator </h1> </div>
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control mt-1" id="img_card_con" v-model="Creator.name_con" readonly>
                        </div>
                        <div class="form-group mt-1">
                            <label for="img_card_con" style="color: #A0A0A0;">Upload Image Creator :</label> <span style="font-size: 15px; color: #666;">ขนาดไฟล์ต้องไม่เกิน 500 KB</span>
...
export default{
    ...
    created() {
        ...
        axios.get(apiURL).then((res) => {
            ...
            this.Name_File = (this.Creator._id || '').substring(0, 10).trim() + '.jpg';
            this.Creator.img_card_con = this.Name_File
        })
    },
    methods: {
    async uploadCreator_img() {
    ...
    try {
        const response = await axios.post('/api_creator/create-creator-img', formData, {
        headers: {
    ...
    let apiURL = `http://localhost:4000/api_creator/update-creator/${this.$route.params.id}`;
            axios.put(apiURL, this.Creator).then((res) => {
                console.log(res);
            }).catch(error => {
                console.log(error)
            })
